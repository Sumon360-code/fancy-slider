...
const createSlider = () => {
  ...
  prevNext.innerHTML = ` 
  <span class="prev" onclick="itemChange(-1)"><i class="fas fa-chevron-left"></i></span>
  <span class="next" onclick="itemChange(1)"><i class="fas fa-chevron-right"></i></span>
  `;
  ...
  if (duration >= 400) {
    ...
    alert('Slider change duration can not be less than .04 second!!');
  }
}
...
searchBtn.addEventListener('click', function () {
  ...
  // handling input field for unexpected search
  if(searchValue.match(letters)){
    getImages(searchValue);
  }else{
   alert("Oops! input cannot be empty! search a valid name");
  }
  sliders.length = 0;
});
